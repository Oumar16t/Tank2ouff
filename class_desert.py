...
class game2:
    def __init__(self):
        ...
        self.font = pygame.font.Font("newfont.ttf",25)
        self.pressed =  {}
    ...
    def start(self):
        ...
        son_niveau2.play(0, 0, 6000)
        son_niveau2.set_volume(0.1)
    ...
    def update(self, screen,time2):
        ...
        if self.ennemie_2_dead!=0 and self.ennemie_dead!=0:
            self.win()
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, game2):
        ...
        self.image = pygame.image.load('tank_visu/tank niveau 2.png')
    ...
    def move_right(self):
        if not self.game2.check_collision(self,self.game2.all_ennemies):
...
class Projectile(pygame.sprite.Sprite):
    ...
    def move(self, time2):
        if time2 < 2:
            ...
            self.a += 0.15
            self.rect.x += self.velocity + (time2*3) - 1
            self.rect.y -= self.velocity + (time2*4) - 2*self.a

        else:
            self.rect.x+=self.velocity*2

        for ennemies in self.player.game2.check_collision(self, self.player.game2.all_ennemies):
            self.remove()
        ...
        for ennemies_2 in self.player.game2.check_collision(self,self.player.game2.all_ennemies_2):
            self.remove()
            son_dommage = pygame.mixer.Sound("sound/degat_tank.mp3")
            son_dommage.play()
...
class ennemies(pygame.sprite.Sprite):
    ...
    def damage(self,degats):
        ...
        if self.health <= 0:
            self.game2.score += 20
            self.game2.all_ennemies.empty()
            self.game2.ennemie_dead = 1
    ...
    def forward(self):
        ...
        if not self.game2.check_collision(self,self.game2.all_players):
            ...
            self.game2.player.damage(self.attack)
...
class projectil_ennemie(pygame.sprite.Sprite):
    ...
    def tir(self):
        ...
        for player in self.ennemie.game2.check_collision(self, self.ennemie.game2.all_players):
...
class ennemies_2(pygame.sprite.Sprite):
    ...
    def damage(self,degats):
        ...
        if self.health <= 0:
            self.game2.score += 20
            self.game2.all_ennemies_2.empty()
            self.game2.ennemie_2_dead = 1
